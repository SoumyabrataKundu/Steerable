...
class Reconstruct:
    ...
    def _unfold(self, patches, grouped=False):
        if grouped:
            outputs = torch.zeros(*patches.shape[0:2], *self.pad_image_shape, device=patches.device)
        else:
            outputs = torch.zeros(patches.shape[1], *self.pad_image_shape, device=patches.device)
