...
class Model(torch.nn.Module):
    def __init__(self, channel, kernel, n_radius, max_m, restricted=False, conv_first=False) -> None:
        ...
        n_theta = 40
        self.network = torch.nn.Sequential(
            snn.SE3Conv(1, [channel]*(max_m+1), kernel, n_radius=n_radius, n_theta=n_theta, conv_first = conv_first),
            snn.SE3Conv([channel]*(max_m+1), [channel]*(max_m+1), kernel, n_radius=n_radius, n_theta=n_theta, restricted=restricted, conv_first = conv_first),
        )
...
def time_compare(channel, kernel, n_radius, max_m, restricted, conv_first):
    ...
    inputs = torch.randn(batch_size, 1, *[resolution]*3, device=device)
    ...
    with open(f"./log1/times_{channel}_{kernel}_{n_radius}_{max_m}_{restricted}_{conv_first}.pkl", 'wb') as file:  
