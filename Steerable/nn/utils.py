...
def get_Fint_matrix(kernel_size, n_radius, n_angle, freq_cutoff, interpolation_type=1, sigma=0.6):
    ...
    if interpolation_type == -1:
        ...
        h = torch.prod(torch.tensor([(kernel_size[i] - 1) / 2 for i in range(len(kernel_size))])) ** (1/len(kernel_size))
        scalar = (torch.arange(1,n_radius+1) * h / (n_radius+1)) ** (len(kernel_size)-1)
        ...
        if len(kernel_size) == 2:
            ...
            Fint1 = torch.einsum('r, mt, rtxy -> mrxy', scalar, SHT, I)
