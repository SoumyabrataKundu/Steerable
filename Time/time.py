...
class Model(torch.nn.Module):
    def __init__(self, channel, kernel, n_radius, max_m, restricted=False, conv_first=False) -> None:
        ...
        self.network = torch.nn.Sequential(
            snn.SE2ConvType1(1, channel, kernel, n_radius=n_radius, n_theta=n_theta, max_m=max_m, padding='same', conv_first = conv_first),
            snn.SE2ConvType2(channel, channel, kernel, n_radius=n_radius, n_theta=n_theta, max_m=max_m, padding='same', restricted=restricted, conv_first = conv_first),
            snn.SE2ConvType2(channel, channel, kernel, n_radius=n_radius, n_theta=n_theta, max_m=max_m, padding='same', restricted=restricted, conv_first = conv_first),            
        )
...
if __name__=='__main__':
    ...
    parser.add_argument("--restricted", type=bool,required=True)
    parser.add_argument("--conv_first", type=bool,required=True)
